...
power and ground. The script also connects Jumperless UART TX/RX to Nano D0/D1
for uploading sketches and reading their serial output.
...
On the development fixture this is **COM11**. The Nano UART/upload interface is
**COM9**; COM10 is the Jumperless menu and COM12 is its machine interface. Port
...
rows 1–8 or the Nano/UART pins it needs, then installs and checks all ten routes.
...
bootloader option if that is what the installed Nano requires. Route checks
